...
Phase 4B 先实现核心类型。Phase 4D 将加入差异严重度、责任人、SLA 和日终汇总。
...
- Bybit / MT5 只提供显式 unsupported shell，真实 Demo 查询属于 Phase 4C。
- Backend Account Snapshot Run 当前按调用触发，尚无日终调度器。
- Position Snapshot 只用于对账，不成为 Formal Position 的计算来源。
- Difference 尚未加入严重度、负责人和 SLA。
- 当前系统仍只允许 Simulation / Fake Gateway。