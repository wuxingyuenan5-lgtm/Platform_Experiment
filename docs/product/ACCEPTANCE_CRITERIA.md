...
5. Strategy management and market pages: real, sample and unavailable data states are distinguished; the six current strategy desks are surfaced from server-side authority rather than frontend sample defaults; accounts, orders, fills, positions, fees and PnL/NAV are traceable to their authoritative facts and sampled external reconciliation is exact; pages show freshness, delay and degraded state.
