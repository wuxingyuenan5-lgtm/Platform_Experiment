...
- Pyright 先覆盖执行 DTO、FinancialFact DTO/Normalization/Repository/Projection Service、Runtime 契约、迁移账本和权威下单边界；每次扩展必须保持所选模块清洁。
...
- `tests/test_architecture_financial_projection_service.py` 阻止正式 Position/PnL/NAV 公式或 Repository 编排重新回流到 API 层，并阻止 Projection Service 依赖 FastAPI 或配置层。
...
- `platform-backend/app/financial_facts.py`、`financial_fact_normalization.py` 与 `financial_projection_service.py` 不得直接导入数据库连接或包含 SQL。
...
- `platform-backend/app/financial_fact_repository.py` 维护 `financial_facts`、`formal_positions`、`formal_pnl_results` 与正式 NAV 的持久化和事务。
- `platform-backend/app/financial_projection_service.py` 是平均成本、已实现交易 PnL、Funding/Swap/Fee/FX 分项、正式 Position/PnL 重建、策略重建审计以及 NAV 覆盖/权益/质量计算的唯一 Owner。
- `platform-backend/app/financial_facts.py` 只负责目录上下文解析、FinancialFact 记录、策略有效性校验、领域错误到 HTTP 的映射和 API 路由，并保留兼容包装器。
- Projection Service 不依赖 FastAPI、配置模块或外部交易场所；所有数据库操作通过 Repository 完成。
- 运营投影服务于交易监控和即时展示，不构成正式会计权威；正式账务必须从不可变事实重建。
- 交易链路不得写入正式投影，正式账务链路不得读取运营投影作为计算输入。
- `tests/test_projection_boundaries.py`、FinancialFact Schema/Normalization/Repository/Projection 架构测试、公式 Golden、Repository 事务测试与结构脚本共同执行回归检查。
