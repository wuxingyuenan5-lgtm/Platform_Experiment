...
| Sensitive identity response cache policy | `platform-backend/app/user_cache_control.py` | `no-store` and legacy proxy-cache prevention for `/auth/**`, `/me/**` and `/users/**`, including rejected requests | Authentication decisions, user data or route permissions |
...
| User and Session persistence | `platform-backend/app/user_repository.py` | User lookup, browser-user persistence, opaque Session persistence, bounded Session count, reset tickets and revocation SQL | HTTP, password verification, menu policy or trading authorization |
...
| Browser Session validation | `platform-backend/app/user_session_auth.py` | Session expiry, idle timeout, auth-version invalidation, stable Session/CSRF error codes, Origin validation and activity throttling | Login credential verification, user administration or API-key validation |
| Public auth and self-account use cases | `platform-backend/app/user_service.py`, `platform-backend/app/user_password_reset.py`, `platform-backend/app/user_logout.py` | Registration, login, current authentication, reauthentication, profile/password/device workflows, reset-ticket consumption and idempotent logout | FastAPI route assembly, administrator target policy, Venue effects or formal accounting |
| Public auth and self-account routes/DTOs | `platform-backend/app/user_routes.py`, `platform-backend/app/user_schemas.py` | `/auth/**` and `/me/**` HTTP contracts, Cookie issuance/clearing, rate-limit routing and service-error mapping | Direct SQL, password hashing or business authorization policy |
| Avatar processing and persistence | `platform-backend/app/user_avatar.py`, `platform-backend/app/user_avatar_repository.py` | Bounded decode, EXIF normalization, WebP re-encoding, opaque filesystem keys, optimistic row updates and transactional audit | Arbitrary external URLs, raw upload persistence or general profile policy |
| Avatar routes/DTOs | `platform-backend/app/user_avatar_routes.py`, `platform-backend/app/user_avatar_schemas.py` | Session-only upload/read/delete contracts and exact HTTP error mapping | Image processing or direct SQL |
| User-administration target policy | `platform-backend/app/user_admin_policy.py` | CEO/technical-lead target and assignable-role rules plus recent-reauth requirements | SQL, HTTP or menu visibility |
| User-administration persistence | `platform-backend/app/user_admin_repository.py` | User search/detail/update, lifecycle/role transitions and administration audit reads within protected transactions | FastAPI or target-role decisions |
| User-administration use cases | `platform-backend/app/user_admin_service.py` | Search, create, approve/reject, edit, role/status, reset-ticket and forced-session-revocation sequencing with transactional audit | Route composition, frontend visibility or hard deletion |
| User-administration routes/DTOs | `platform-backend/app/user_admin_routes.py`, `platform-backend/app/user_admin_schemas.py` | `/users/**` management HTTP contracts, permission dependencies and response masking selection | Direct SQL, password generation or target policy redefinition |
| Member holding Decimal policy | `platform-backend/app/member_holding_decimal.py` | Plain non-negative Decimal parsing, canonical strings and exact derived reporting values | Currency conversion, formal accounting or SQL |
| Member holding and NAV persistence | `platform-backend/app/member_holding_repository.py` | Holding/NAV SQL, optimistic versions, source constraints and fund catalog reads | HTTP, permission policy or derived Decimal formulas |
| Member holding and NAV use cases | `platform-backend/app/member_holding_service.py` | Self/admin scope, member-target validation, NAV stale/unavailable semantics, recent reauthentication and transactional audit | Subscription/redemption accounting, execution or strategy NAV substitution |
| Member holding routes/DTOs | `platform-backend/app/member_holding_routes.py`, `platform-backend/app/member_holding_schemas.py` | Session-only self/admin holding and NAV HTTP contracts; browser writes restricted to `manual_admin` | Import pipelines, direct SQL or floating-point calculations |
...
| Frontend user-system API boundary | `admin-risk/src/api/platform/userSystem.ts`, `admin-risk/src/api/platform/memberHoldings.ts` | Same-origin Cookie clients, memory-only/multi-tab CSRF, structured DTOs and stable-code Session invalidation | Persistent authentication tokens, role authorization or floating-point holding calculations |
| Frontend browser access registry | `admin-risk/src/access/userAccess.ts`, `admin-risk/src/access/routeAccess.ts` | Exact permission matching and one permission-aware route/menu tree | Wildcard expansion, backend authorization or duplicate menu facts |
| Frontend browser Session state | `admin-risk/src/store/modules/user.ts`, `admin-risk/src/router/guard/permissionGuard.ts` | Session hydration, permission-store population, state reset and centralized dynamic-route installation | Credential persistence, backend role inference or unfiltered route installation |
...
| Migration ledger | `platform-backend/app/schema_migrations.py` | Ordered additive migrations, including execution policy, user identity/session and member holding/NAV schema, with immutable checksums | Editing an applied migration |
