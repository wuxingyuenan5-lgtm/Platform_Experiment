...
| Venue Reconciliation Runtime client | `platform-backend/app/venue_reconciliation_runtime_client.py` | Configured Runtime GET transport, URL/parameter/timeout application and transport-error boundary | FastAPI error responses, persistence, FinancialFact import, difference rules or route orchestration |
| Venue Reconciliation orchestration | `platform-backend/app/venue_reconciliation.py` | Compatibility exports, FinancialFact import, repository/policy/Runtime-client coordination, FastAPI error mapping and routes pending Service/route extraction | Direct SQL/DDL/row mapping, direct Runtime HTTP or duplicate DTO/policy definitions |
