...
| Cross-spread live read transport | `platform-backend/app/cross_spread_live_read_client.py` | Configured Runtime transport and typed live Instrument/Position/Order reads for cross-spread safety checks | FastAPI routes, SQL, venue SDKs or execution decisions |
...
| Bybit acceptance reads | `execution-runtime/app/bybit_acceptance_adapter.py` | Route-independent Order/Fill reads, bounded order list, live instrument specification and API-key readiness mapping | Platform business rules, write authorization or MT5 behavior |
