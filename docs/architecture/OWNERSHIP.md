...
| Member holding and NAV use cases | `platform-api/app/member_holding_service.py` | Self/admin scope, member-target validation, NAV stale/unavailable semantics, recent reauthentication and transactional audit | Subscription/redemption accounting, execution or strategy NAV substitution |
...
| EOD Reconciliation facade | `platform-api/app/eod_reconciliation.py` | Per-call dependency wiring, compatibility delegates, exact service-error-to-HTTP mapping and routes pending dedicated route-module extraction | Cross-domain use-case sequencing, direct SQL/DDL or duplicate Policy decisions |
