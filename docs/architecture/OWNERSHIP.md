...
| Frontend user-system API boundary | `admin-risk/src/api/platform/userSystem.ts`, `admin-risk/src/api/platform/memberHoldings.ts` | Same-origin Cookie clients, in-memory CSRF, structured user/holding DTOs and 401 session-memory clearing | Persistent authentication tokens, role authorization or floating-point holding calculations |
