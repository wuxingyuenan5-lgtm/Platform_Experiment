...
Each existing task pack has separate Required and Optional token budgets in `scripts/context-for.py`. `PACK_BUDGETS` is the sole numeric budget authority, while file paths remain owned by the existing `PACKS` definitions.
