...
## Delivery boundaries
Governance is a risk ceiling, not mandatory ceremony. Use the smallest reliable team shape. One workflow, public contract, migration chain or shared file set has one implementation owner. Long-term roles do not consume implementation slots, but they may not bypass single-writer rules by modifying business source.
The safe upper bound is four active temporary agents: at most two implementation and at most two read-only. The default is one implementation agent. A second implementation agent is allowed only when the technical lead records concrete independence evidence before dispatch: disjoint write sets, no unfinished dependency, independent tests and independent rollback. Do not split work merely to use concurrency capacity.
...
Implementation-only fields are required only for implementation work: `Implementation owner`, `Branch`, `Worktree`, `Base commit`, `Write set`, `Shared workflow, public contract, migration chain or file set`, `Dependencies`, `Independent test`, `Rollback boundary` and `Parallel decision`.
...
Dispatch is system-file-first. The startup message passes only task identifier, authority commit, Context Pack, task-card or version-control-file path, and any new owner delta. If a new agent cannot cold-start from repository authority, the task card is incomplete.

## Token lifecycle and status events
Token fields are management indicators, not API billing. `Token delta` must equal `Token current - Token baseline`. `Control-plane token delta` may not exceed 30 percent of total Token delta. The repository validator checks declarations; it is not Codex runtime hard rate limiting.
