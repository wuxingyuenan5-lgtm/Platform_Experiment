...
- Repository delivery default: use Local execution on `main` with one implementation writer and serial work. Do not create task branches or linked worktrees for ordinary work; use them only for explicitly approved high-risk recovery or proven parallel writes, then close them. Read-only roles do not create branches or worktrees. Project Hooks are optional and disabled by default; repository operation must not depend on them.
