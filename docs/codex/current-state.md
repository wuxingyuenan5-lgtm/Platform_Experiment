...
- Stable baseline: Platform `0.11.1`.
- Current candidate target: Platform `0.11.2`.
...
- Platform 0.11.2 is the next local candidate after the 0.11.1 main snapshot. The Platform 0.11.2 candidate scope includes final independent acceptance, browser-access regression hardening and Capability alignment; it does not reopen accepted product restoration, business behavior or controlled-live core direction.
- Its included scope is two local, CEO-operated strategy closures: the existing Bybit `XAUTUSDT` and MT5 `XAUUSD+` cross-venue gold spread, and same-account Bybit funding carry using a CEO-specified perpetual-short plus spot-long pair. Both reuse one bounded two-leg execution, idempotency, ledger, reconciliation and recovery foundation. Funding carry supports CEO-specified symbols and quantities, multiple held pairs, perpetual-first bounded PostOnly execution for open and close, and at least one verified funding settlement. The owner additionally authorized three named Bybit read-only strategy-account migrations: bottom-fishing, short-term trader A and short-term trader B. They expose account facts only and cannot submit, amend or cancel orders. The candidate excludes automatic symbol selection, automatic sizing or rebalancing, reverse carry, cross-venue funding carry, unattended trading, external client funds, multi-person fund operation, production deployment and home-abroad spread.
...
- `AGENTS.md` is the normal project entrypoint. Governance documents, Context Packs, task cards, and module instructions are read only when explicitly requested or directly required by the affected surface. Context estimates are guidance, not execution gates.
...
- The owner does not require fixed position, notional, daily-volume, batch-count or loss caps for the founder-owned local test accounts; the former 1-ounce acceptance size was a test convention rather than a permanent product risk limit. Existing Platform and Runtime fixed-notional-cap code remains a controlled-live readiness blocker and is unchanged for legacy compatibility in the rule-alignment slice. A separately authorized behavior change must align it before Live Write without weakening instruction-bounded quantity, one-batch identity, cumulative-fill ceilings, bounded chase, global execution serialization, Kill Switch, fail-closed unknown handling, reconciliation or forced read-only reset. This decision is not Live Write authority.
- Funding partial fills use incremental hedging: each newly proven perpetual cumulative-fill increment permits only its CEO-specified proportional spot increment while the bounded perpetual remainder remains active. Duplicate events cannot duplicate spot fills; expiry or the approved chase boundary cancels the remainder, and unknown cancellation or execution state stops further side effects. Initial local-test PostOnly settings are owner-approved at a 15-second total chase TTL, one-second evaluation cadence, at most five amend or cancel-repost mutations, and at least one tick of price change before mutation. They are configuration rather than exchange facts or permanent production defaults; any pre-live change requires evidence and owner approval.
...
- Windows local lifecycle uses only Platform Web (4373), Platform API (8000) and Execution Runtime (8100). Startup records only successfully healthy services; status derives runtime state from listeners and health checks; stop acts only on matching managed records.
...
- Repository delivery default: use Mode 1, with one Agent directly completing ordinary work on `main`. Do not automatically create leads, sub Agents, task cards, branches, linked Worktrees, locks, or monitors. Mode 2 and Mode 3 require explicit Owner selection. Project hooks, runtime injection, and agent profiles are not configured.
