...
3. 资金费策略声明了永续优先、增量现货对冲和 PostOnly Chase，但当前工作区实现仍构造两个市价腿，共享批次代码也没有对应的资金费增量释放实现。该差异是进入任何真实写入前必须关闭的 P0 安全缺口。
...
- Bybit 同账户资金费套利：CEO 指定永续空头与现货多头品种和两腿数量。
...
第一阶段保持一个活动写入批次的保守策略，不追求并行吞吐。已经确认终态且无未知外部订单的历史 Batch 可以释放租约；任何 `result_unknown`、取消不确定或残余敞口继续阻止新写入。
...
- 实现永续优先的有界 PostOnly Chase。
- 按去重后的权威永续累计成交增量释放 Spot。
- 证明数量步长、余数、重复事件、取消/成交竞态、重启和未知结果。
- 在全部离线测试通过前保持真实资金费 Live Write 不可用。

### 阶段 3：黄金策略迁移与通用编排

- 把黄金数量换算和腿依赖迁入冻结 Plan。
- 删除共享 Execution Batch 中按策略名称分支的逻辑。
- 保留黄金当前 Market/FOK/PostOnly、关闭身份和回滚安全语义。

### 阶段 4：Position Group 与统一 Read Model
...
- 交付一条命令完成 Offline E2E 的启动、测试、报告和关闭。
- 实现 Live Read-Only 与 Controlled Live 的机器可读会话范围。
- 下线未被消费者使用的旧 Strategy API 和 Catalog 内重复 Trade Command 实现。
- 完成文档、架构边界和回归检查。
