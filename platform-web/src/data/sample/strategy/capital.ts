...
function makeCapitalMetricCurves(volPoints: number[], sharpePoints: number[]) {
  return [
    makeMetricCurve(
      '波动率',
      volPoints,
      `${((volPoints[volPoints.length - 1] ?? 0) * 100).toFixed(2)}`,
      'negative',
      '%',
    ),
    makeMetricCurve(
      '夏普比率',
      sharpePoints,
      (sharpePoints[sharpePoints.length - 1] ?? 0).toFixed(2),
      'negative',
    ),
  ];
}
...
export const strategyCapitalProfiles = {
  ...
  dip: {
    overview: [
      ...
      {
        label: '过去7天盈亏',
        value: '+24,680.30 (+1.63%)',
        note: '回补阶段收益继续释放',
        tone: 'positive',
      },
      {
        label: '过去30天盈亏',
        value: '+66,540.18 (+4.39%)',
        note: '近月组合净值修复明显',
        tone: 'positive',
      },
      {
  ...
  shortLineTraderL: {
    ...
    specialRulePanel: {
      ...
      alerts: [
        {
          time: '2026-07-12 13:12:33',
          text: 'BTC 追单后波动放大，触发一次风控减仓提醒。',
          tone: 'neutral',
        },
        {
          time: '2026-07-12 10:18:44',
          text: '黄金止盈单撤回，已发送人工复核消息。',
          tone: 'neutral',
        },
        {
          time: '2026-07-12 09:36:08',
          text: 'IF2609 保护止损已同步挂出，风控检测通过。',
          tone: 'positive',
        },
      ],
    },
...
strategyCapitalProfiles.shortLineTraderW.overview =
  strategyCapitalProfiles.shortLineTraderW.overview.map((item) => ({
    ...item,
    note: item.note.replaceAll('短线交易员A', '短线交易员B'),
  }));
...
if (strategyCapitalProfiles.shortLineTraderW.specialRulePanel) {
  strategyCapitalProfiles.shortLineTraderW.specialRulePanel.title = '短线交易员B专属风控规则';
}
