    ...
    <div class="news-layout">
      ...
      <div class="section-content">
        ...
        <RestoredProductSurface
          ...
          :actionable="false"
          message="新闻 Provider 尚未配置；标题、摘要和影响标签为明确标记的样例内容。"
        >
        ...
        <RestoredProductSurface
          ...
          as-of="2026-08-05 · 非实时"
          :actionable="false"
          message="活动利率、额度和期限不是实时事实，不构成推荐或收益承诺。"
        >
          <section class="wealth-grid">
            <article v-for="item in wealthItems" :key="item.title">
              <span>{{ item.type }}</span>
              <h2>{{ item.title }}</h2>
              <strong>{{ item.rate }}</strong>
              <p>{{ item.note }}</p>
              <button disabled>仅查看外部参考</button>
            </article>
          </section>
          <p class="disclaimer">
            示例内容不可申购、不可执行；外部页面由第三方维护，不属于 Platform 正式数据。
          </p>
        </RestoredProductSurface>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  const sectionTabs: Array<{
    ...
    },
    {
      key: 'news',
      label: '新闻整理',
      path: '/news-calendar/news',
      icon: FileSearchOutlined,
    },
    {
      key: 'wealth',
      label: '理财信息',
      path: '/news-calendar/wealth',
      icon: FundOutlined,
    },
  ];
  const newsItems = [
    {
      ...
      time: '08:45',
      title: '成交扩散与风格切换观察',
      summary: '关注宽基上涨是否伴随成交额和行业广度同步改善。',
      impact: '中等影响',
