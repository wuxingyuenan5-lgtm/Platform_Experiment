<template>
  <section class="funding-chart-panel" data-testid="funding-chart-panel">
    <header>
      <div
        ><span>资金费率路径</span><h2>{{ exchange }} · {{ symbol }}</h2></div
      >
    ...
    <div class="chart" role="img" :aria-label="`${symbol} 资金费率样例图表`">
      ...
      >
        <polyline
          :points="toPoints(seriesItem.values)"
          fill="none"
          :class="`series-${index}`"
          stroke-width="3"
        />
    ...
    <footer>
      <span v-for="(seriesItem, index) in series" :key="seriesItem.label"
        ><i :class="`series-${index}`"></i>{{ seriesItem.label }}</span
      >
...
<script setup lang="ts">
  ...
  function toPoints(values: number[]): string {
    ...
    return values
      .map(
        (value, index) =>
          `${((index / Math.max(values.length - 1, 1)) * 640).toFixed(1)},${(
            160 -
            ((value - min) / range) * 130
          ).toFixed(1)}`,
      )
