<template>
  <section class="funding-market-board" data-testid="funding-market-board">
    <header>
      ...
      <div class="filters">
        <select :value="selectedSymbol" @change="emitValue('update:selectedSymbol', $event)">
          <option v-for="symbol in ['BTC', 'ETH', 'SOL']" :key="symbol" :value="symbol">{{ symbol }}</option>
        </select>
        ...
        <select :value="selectedResolution" @change="emitValue('update:selectedResolution', $event)">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>交易所</th><th>币种</th><th>资金费率</th><th>基差</th><th>流动性</th></tr></thead>
        <tbody>
          <tr v-for="row in filteredRows" :key="`${row.exchange}-${row.symbol}`">
            <td>{{ row.exchange }}</td><td>{{ row.symbol }}</td><td>{{ row.rate }}</td><td>{{ row.basis }}</td><td>{{ row.liquidity }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
<script setup lang="ts">
  ...
  const props = defineProps<{
    rows: Array<{ exchange: string; symbol: string; rate: string; basis: string; liquidity: string }>;
  ...
  const filteredRows = computed(() => props.rows.filter((row) => row.symbol === props.selectedSymbol));
...
<style scoped lang="less">
  .funding-market-board { display: grid; gap: 14px; padding: 18px; border: 1px solid #e1e7ef; border-radius: 14px; background: #fff; }
  header { display: flex; justify-content: space-between; gap: 16px; align-items: flex-end; }
  header span { color: #718095; font-size: 11px; }
  h2 { margin: 4px 0 0; font-size: 18px; }
  .filters { display: flex; gap: 8px; flex-wrap: wrap; }
  select { height: 34px; padding: 0 9px; border: 1px solid #dce3eb; border-radius: 8px; background: #fff; }
  ...
  @media (max-width: 720px) { header { flex-direction: column; align-items: stretch; } }
</style>
