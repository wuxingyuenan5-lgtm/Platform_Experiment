<template>
  <div class="spread-page" data-testid="spread-original-structure">
    ...
    <template v-if="activeSection === 'analysis'">
      ...
      <DomesticOverseasMarketInsight
        v-if="variant === 'domesticOverseas'"
        :selected-metal="domesticMetal"
      />
    ...
    <DomesticOverseasExecutionWorkspace
      v-else-if="variant === 'domesticOverseas'"
...
<script setup lang="ts">
  ...
  import DomesticOverseasExecutionWorkspace from './components/DomesticOverseasExecutionWorkspace.vue';
  ...
  const spreadSeries = [
    18.2, 17.9, 16.8, 14.4, 12.2, 10.9, 8.1, 7.4, 8.6, 10.3, 12.5, 14.8, 16.7, 18.1, 19.4, 18.8,
    18.1, 17.3,
  ];
  const goldPriceSeries = [
  ...
  watch(
    () => props.selectedVenue,
    (v) => (selectedVenue.value = v),
    { immediate: true },
  );
  watch(
    () => props.leftLegSymbol,
    (v) => (leftLegSymbol.value = v),
    { immediate: true },
  );
  watch(
    () => props.rightLegSymbol,
    (v) => (rightLegSymbol.value = v),
    { immediate: true },
  );
  watch(
  ...
  watch(
    ...
    (value) => {
      if (value === 'domesticOverseas') {
        leftLegSymbol.value = 'SHFE.au2606';
      }
    },
    { immediate: true },
  );
