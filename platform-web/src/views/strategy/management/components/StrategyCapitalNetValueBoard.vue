    ...
    <header class="board-header">
      ...
      <div class="board-controls">
        <select v-model="metricKey">
          <option v-for="item in curve.metricOptions" :key="item.key" :value="item.key">{{
            item.label
          }}</option>
        </select>
        <select v-model="periodKey">
          <option v-for="item in curve.periodOptions" :key="item.key" :value="item.key">{{
            item.label
          }}</option>
        </select>
...
<script setup lang="ts">
  ...
  import type { StrategyCapitalCurveConfig } from '@/data/sample/strategy';
      ...
      series: [
        {
          ...
          areaStyle: {
            color:
              modeKey.value === 'fixed' ? 'rgba(138, 99, 210, .08)' : 'rgba(74, 124, 255, .08)',
          },
