...
<script lang="ts" setup>
  ...
  import { getMenuList, postRoleListByPage } from '@/api/demo/system';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (unref(treeData).length === 0) {
      treeData.value = (await getMenuList())?.data as any as TreeItem[];
    }
  ...
  async function postRoleListByPageFn(params: any) {
    try {
      ...
      loading.value = true;
      const res = await postRoleListByPage(_params);
      if (res.retCode == 0) {
