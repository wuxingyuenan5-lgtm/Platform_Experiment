...
<script lang="tsx" setup>
  ...
  import { getProductNavList } from '@/api/data/product';
  import {
    timeOptions3,
  ...
  function getProductNavListFn(params?: any) {
    loadingContent.value = true;
    const _params = {
      ...params,
      tradingTimeFilter: !!searchInfo.tradingTimeFilter,
      is_dynamic: !!searchInfo.is_dynamic,
    };
    getProductNavList(_params)
      .then((res) => {
        if (res?.retCode == 0) {
          chartData.value =
            res?.data?.map((item: any) => {
              return {
                ...item,
                current_drawdown: (item.current_drawdown * 100).toFixed(2),
              };
            }) || [];
          initChart();
        }
  ...
  watch(
    ...
    (curV) => {
      if (curV && curV.value) {
        getProductNavListFn({ product_code: curV.value, ...searchInfo });
      }
    },
    { immediate: true },
  );
  watchDebounced(
    () => searchInfo,
    () => {
      getProductNavListFn({ product_code: props.product.value, ...searchInfo });
    },
