    ...
    <main class="ai-page">
      ...
      <section class="ai-layout">
        <aside class="panel sessions">
          ...
          <button v-for="item in sessions" :key="item.title">
        ...
        <section class="panel conversation">
          ...
          <div class="composer">
            ...
            <button v-if="canWrite" disabled>运行分析</button>
            <b v-else>只读权限</b>
          </div>
        </section>
        ...
        <aside class="panel evidence-panel">
          ...
          <ul>
            <li v-for="item in evidence" :key="item"><i></i>{{ item }}</li>
          </ul>
...
<script setup lang="ts">
  ...
  const canWrite = computed(() =>
    hasPermission(userStore.getAuthentication?.permissions || [], 'research.write'),
  );
  const sessions = [
    { title: '跨资产晨报', note: '模板' },
  ...
  const evidence = [
    '标注数据来源与时间',
    '区分事实、计算和推断',
    '展示 live/sample/unavailable/error',
    '禁止无门禁的交易执行',
  ];
</script>
...
<style scoped>
  ...
  .ai-head {
    ...
    padding: 24px;
    border-radius: 16px;
    background: linear-gradient(135deg, #151a2e, #293a68);
    color: #fff;
  }
