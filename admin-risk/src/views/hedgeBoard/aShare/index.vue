  ...
  <PageWrapper title="对冲基金看板">
    <div class="a-share-board">
      <section class="board-toolbar">
        ...
        />
        <button type="button" class="refresh-button" :disabled="dashboardLoading" @click="loadDashboard">
          {{ dashboardLoading ? '刷新中…' : '刷新数据' }}
        </button>
      </section>

      <nav class="research-subnav" aria-label="A股研究模块导航">
        <button v-for="(item, index) in sectionNav" :key="item.id" type="button" @click="jumpTo(item.id)">
      ...
      <section id="a-share-overview" class="overview-card">
        ...
        <div class="overview-grid">
          <article v-for="row in overviewRows" :key="row.code">
            <div class="overview-card__head"><span>{{ row.code }}</span><b :class="toneClass(row.return1dPct)">{{ formatPct(row.return1dPct) }}</b></div>
...
<script setup lang="ts">
  ...
  function sparkPoints(values: Array<string | number>) {
    ...
    return series.map((value, index) => `${(index / Math.max(series.length - 1, 1) * 100).toFixed(2)},${(26 - (value - min) / range * 22).toFixed(2)}`).join(' ');
  }
</script>
...
<style scoped lang="less">
  .a-share-board {
    ...
    --strategy-accent-ring: rgba(67, 56, 202, .18);
    --strategy-up: #ef4444;
    ...
    --strategy-shadow: 0 8px 24px rgba(15, 23, 42, .06);
    --strategy-shadow-soft: 0 4px 16px rgba(15, 23, 42, .045);
    display: grid;
    gap: 14px;
    min-width: 0;
    padding-bottom: 32px;
  ...
  .board-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
  .refresh-button { min-height: 36px; padding: 0 13px; border: 1px solid var(--strategy-border); border-radius: 9px; background: var(--strategy-surface); color: var(--strategy-text-2); cursor: pointer; }
  .refresh-button:disabled { opacity: .65; cursor: wait; }
  .research-subnav { position: sticky; top: 0; z-index: 5; display: flex; gap: 8px; overflow-x: auto; padding: 8px 0; background: var(--strategy-bg); }
  .research-subnav button { display: inline-flex; align-items: center; gap: 7px; min-width: max-content; padding: 9px 12px; border: 1px solid var(--strategy-border); border-radius: 999px; background: var(--strategy-surface); color: #111827; cursor: pointer; box-shadow: var(--strategy-shadow-soft); }
  .research-subnav span, .research-subnav strong { color: #111827; font-size: 14px; font-weight: 800; }
  .dashboard-error { padding: 11px 13px; border-radius: 10px; background: rgba(239, 68, 68, .08); color: #dc2626; }
  .overview-card { padding: 18px; border: 1px solid var(--strategy-border); border-radius: var(--strategy-radius-card); background: var(--strategy-surface); box-shadow: var(--strategy-shadow-soft); }
  .section-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 15px; }
  .section-header p { margin: 0 0 3px; color: var(--strategy-text-4); font-size: 11px; font-weight: 800; letter-spacing: .12em; }
  .section-header h2 { margin: 0; color: var(--strategy-text-1); font-size: 18px; }
  .overview-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 11px; }
  .overview-grid article { min-width: 0; padding: 13px; border: 1px solid var(--strategy-border); border-radius: 11px; background: var(--strategy-surface-2); }
  .overview-card__head { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
  .overview-card__head span { color: var(--strategy-text-4); font-size: 12px; }
  .overview-grid article > strong { display: block; margin-top: 8px; color: var(--strategy-text-1); }
  .overview-grid article > em { display: block; margin-top: 3px; color: var(--strategy-text-2); font-size: 18px; font-style: normal; font-variant-numeric: tabular-nums; }
  .overview-grid svg { width: 100%; height: 28px; margin-top: 8px; }
  .overview-grid polyline { fill: none; stroke: #64748b; stroke-width: 2; vector-effect: non-scaling-stroke; }
  .overview-grid polyline.is-positive { stroke: var(--strategy-up); }
  .overview-grid polyline.is-negative { stroke: var(--strategy-down); }
  ...
  @media (max-width: 680px) { .board-toolbar, .section-header { align-items: flex-start; flex-direction: column; } .overview-grid { grid-template-columns: 1fr; } }
</style>
