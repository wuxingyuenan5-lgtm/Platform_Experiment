        ...
        <article class="emotion-panel emotion-panel--table">
          ...
          <div class="table-shell">
            <table>
              <thead
                ><tr
                  ><th class="is-left">名称 / 代码</th><th>连板</th
                  ><th class="is-right">成交额</th></tr
                ></thead
              >
              <tbody>
                <tr v-for="row in data.leaders" :key="row.securityCode">
                  <td class="is-left"
                    ><strong>{{ row.securityName }}</strong
                    ><small>{{ row.securityCode }}</small></td
                  >
                  <td>{{ row.boardCount }}板</td>
                  <td class="is-right">{{ formatMoney(row.turnoverYuan) }}</td>
                </tr>
                <tr v-if="!data.leaders.length"
                  ><td colspan="3" class="empty-cell">暂无二板及以上个股</td></tr
                >
              </tbody>
            </table>
          </div>
...
<script setup lang="ts">
  ...
  const metrics = computed(() => {
    ...
    return [
      ...
      {
        label: '最高连板',
        value: `${value.highestBoardCount}板`,
        note: '涨停池最高 lbc',
        tone: '',
      },
      ...
      {
        label: '封板率',
        value: formatPct(value.sealRatePct),
        note: '涨停 /（涨停＋炸板）',
        tone: 'is-positive',
      },
      {
        label: '炸板率',
        value: formatPct(value.breakRatePct),
        note: '炸板 /（涨停＋炸板）',
        tone: 'is-negative',
      },
      {
        label: '晋级率',
        value: formatPct(value.promotionRatePct),
        note: '今日二板+ / 昨日涨停',
        tone: '',
      },
    ];
  ...
  const maxLadderCount = computed(() =>
    Math.max(1, ...(data.value?.ladder || []).map((row) => row.stockCount)),
  );
...
<style scoped lang="less">
  ...
  .emotion-metrics article {
    padding: 13px;
    border: 1px solid var(--strategy-border);
    border-radius: 11px;
    background: var(--strategy-surface-2);
  }
  .emotion-metrics span,
  .emotion-metrics small {
    display: block;
    color: var(--strategy-text-3);
  }
  .emotion-metrics strong {
    display: block;
    margin: 7px 0 3px;
    color: var(--strategy-text-1);
    font-size: 21px;
  ...
  .ladder-track {
    ...
    overflow: hidden;
    border-radius: 999px;
    background: var(--strategy-border);
  }
  .ladder-track i {
    display: block;
    height: 100%;
