<template>
  <div class="component-background h-full">
    <!-- header  -->
...
<script lang="tsx" setup>
  ...
  const showLen = computed(() => {
    return tabVal.value == TabType.BUYSELL ? 10 : 20;
  });
  ...
  function renderItem(type: OrderType, data: any) {
    ...
    return (
      <div class={`flex px-3 leading-6 order-item-${type}`}>
        ...
        <div class="text-right flex-1">{_third || '--'}</div>
      </div>
    );
  }
