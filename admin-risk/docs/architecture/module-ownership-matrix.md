...
| 金融 AI | 研究问答、信息归纳、情景推演、策略 Memo、分析结果容器 |
...
| 实时交易行情 | 策略／交易平台、对冲基金看板只读 | 风险管理和金融 AI 按权限读取 | Market Data | Market Data 服务 |
| 宏观、资产和公司研究数据 | 对冲基金看板 | 交易平台有限引用；金融 AI 分析 | Research Data | Research Data 服务 |
| 新闻、宏观日历和理财内容 | 新闻日历与理财 | 首页、看板和金融 AI 引用 | Content and Calendar | 内容与日历数据源或维护入口 |
...
| 高风险审批和双人复核 | 风险管理相关管理页面或目标业务页面 | 各模块发起并读取审批 | Approval and Control | Approval 服务；目标命令由原领域执行 |
| 对账和数据修正 | 策略管理、风险管理／数据监控 | 相关业务模块读取结果 | Reconciliation／Data Quality | 对账服务；修正通过授权命令写回主责领域 |
| 数据源和同步质量 | 风险管理／数据与服务监控 | 看板、策略和金融 AI 读取状态 | Data Quality／Integration | 数据质量和同步服务 |
...
| AI 分析任务和结果 | 金融 AI | 看板、新闻和策略提供授权数据 | AI Orchestration／Research Data | AI 任务和结果服务 |
