...
本文定义短线交易员 L 策略账户的业务定位、独立身份、外部执行来源、账户和交易归属、交易日与日界线、订单成交持仓、EconomicEvent、日内和区间 PnL、风险额度、违规事件、对账、数据质量、策略管理 Read Model、外部组件候选和验收标准。
...
短线交易员 L 是纳入平台统一统计、风险观察、数据核对和复盘的独立短线方向性交易策略账户。
...
8. L 与 W 的正式差异必须进入各自 StrategyVersion 或 ExternalExecutionProfile。
...
ExternalExecutionProfile 用于描述 L 的外部执行环境和归属规则，不保存密码、Secret 或完整登录凭证。
...
- `GetShortLineTraderLSummary`。
- `GetShortLineTraderLPnlView`。
- `GetShortLineTraderLAccountView`。
- `GetShortLineTraderLPositionView`。
- `GetShortLineTraderLOrderView`。
- `GetShortLineTraderLTradeCycleView`。
- `GetShortLineTraderLRiskView`。
- `GetShortLineTraderLViolationView`。
- `GetShortLineTraderLReconciliationView`。
...
- `AllocateExternalExecutionToShortLineTraderL`。
- `ConfirmShortLineTraderLViolation`。
- `ResolveShortLineTraderLViolation`。
...
- `ShortLineTraderLDataSynchronized`。
- `ShortLineTraderLOrderImported`。
- `ShortLineTraderLPositionUpdated`。
- `ShortLineTraderLRiskChanged`。
- `ShortLineTraderLViolationDetected`。
- `ShortLineTraderLViolationResolved`。
- `ShortLineTraderLReconciliationCompleted`。
- `ShortLineTraderLPnlRecalculated`。
...
当前不使用 vn.py 执行 L 的交易，也不使用其 PnL、账户和风险对象替代平台模型。
...
1. L 的正式 Broker／Exchange／Terminal。
