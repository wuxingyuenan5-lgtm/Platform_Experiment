...
def _validated_execution_events(
    ...
    *,
    ...
    if not events:
        raise HTTPException(status_code=502, detail="Runtime event batch is empty")
    ...
    for raw_event in events:
        ...
        if previous is not None:
            if previous != event:
                raise HTTPException(
                    status_code=502, detail="Runtime event identity has conflicting payload"
                )
            continue
        unique[event_id] = event
        event_type = str(event["event_type"])
        if event_type in {"order_filled", "order_rejected"}:
            terminal_types.add(event_type)
        external_order_id = event.get("external_order_id")
        if external_order_id is not None:
            external_order_ids.add(str(external_order_id))

    if terminal_types == {"order_filled", "order_rejected"}:
        raise HTTPException(status_code=502, detail="Runtime event batch has conflicting outcomes")
    if len(external_order_ids) > 1:
        raise HTTPException(status_code=502, detail="Runtime event batch has conflicting order IDs")
...
def apply_execution_events(
    ...
    *,
    ...
    validated_events = _validated_execution_events(
        events,
        expected_command_id=expected_command_id,
...
def validate_execution_event(
    ...
    *,
    ...
    if event.get("event_type") == "order_filled":
        if event.get("fill_price") is None or event.get("fill_quantity") is None:
            raise HTTPException(status_code=502, detail="Runtime fill event is incomplete")
        if Decimal(str(event["fill_price"])) <= 0 or Decimal(str(event["fill_quantity"])) <= 0:
            raise HTTPException(status_code=502, detail="Runtime fill event is invalid")


def _record_fill_and_update_operational_projections(
    ...
    *,
    ...
    existing = db.execute(
        """
        SELECT order_id, account_id, instrument_id, side, quantity, price, occurred_at
        FROM fills
        WHERE id = ?
        """,
        (fill_id,),
    ...
    if existing is not None:
        ...
        if existing_values != fill_values:
...
def record_fill_and_update_operational_projections(
    *,
    ...
    with connection() as db:
        return _record_fill_and_update_operational_projections(
            db,
            fill_id=fill_id,
            order_id=order_id,
            request=request,
