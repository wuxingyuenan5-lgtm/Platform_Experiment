...
def _membership() -> ShenwanMembership:
...
def test_dashboard_all_provider_failures_are_unavailable_not_empty(
    ...
    modules = (result.market_detail, result.breadth, result.shenwan, result.emotion)
    assert all(module.meta.status == "error" for module in modules)
    assert all(module.data is None for module in modules)
