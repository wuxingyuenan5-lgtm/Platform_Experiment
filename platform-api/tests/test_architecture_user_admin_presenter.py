...
def test_user_admin_presenter_boundary() -> None:
    ...
    assert service.count("from app.user_admin_presenter import") == 3
    assert "admin_audit_response as _audit_response" in service
    assert "admin_user_detail as _detail" in service
    assert "admin_user_summary as _summary" in service
