...
_E2E_ROOT = _REPOSITORY_ROOT / ".e2e" / "platform-visual"


def _required_environment(name: str) -> str:
    value = os.environ.get(name, "").strip()
    if not value:
        raise RuntimeError(f"{name} is required for the platform visual E2E bootstrap")
    return value
...
def _reset_test_data() -> None:
    if os.environ.get("PLATFORM_VISUAL_E2E_RESET") != "1":
        return
    if _E2E_ROOT.is_symlink():
        raise RuntimeError("Refusing to reset a symlinked platform visual E2E root")
    if _E2E_ROOT.exists():
...
def main() -> int:
    ...
    password = _required_environment("E2E_SHARED_PASSWORD")
    accounts = seed_demo_users(password=password, prefix="e2e", refresh_existing=True)
    print("Seeded isolated full-platform visual accounts:")
